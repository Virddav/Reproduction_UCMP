...
for ftype in [1, 2, 3, 4]:
    ...
    if ftype == 1:
        ...
        ax.set_ylabel("Avg. ToR-to-host link util.", fontsize=21)
        if config["legend"]:
            plt.legend(handles=legends, loc='upper center',ncol=6,bbox_to_anchor=(0.5, 1.15), fontsize=18)
    elif ftype == 2:
        ax.set_xlabel("Time ($ms$)", fontsize=26)
        ax.set_ylabel("Avg. ToR-to-ToR link util.", fontsize=21)
        if config["legend"]:
            plt.legend(handles=legends, loc='upper center',ncol=6,bbox_to_anchor=(0.5, 1.15), fontsize=18)
    elif ftype == 3:
        ax.set_xlabel("Avg. ToR-to-host link util.", fontsize=21)
        plt.legend(handles=legends, loc='upper center',ncol=6,bbox_to_anchor=(0.5, 1.15), fontsize=18)
        if config["legend"]:
            ax.set_ylabel("CDF", fontsize=26)
    elif ftype == 4:
        ax.set_xlabel("Avg. ToR-to-ToR link util.", fontsize=21)
        ax.set_ylabel("CDF", fontsize=26)
        if config["legend"]:
            plt.legend(handles=legends, loc='upper center',ncol=6,bbox_to_anchor=(0.5, 1.15), fontsize=18)
